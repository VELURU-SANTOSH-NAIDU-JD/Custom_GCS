...
QGCApplication::QGCApplication(int &argc, char *argv[], bool unitTesting, bool simpleBootTest)
    : QApplication(argc, argv), _runningUnitTests(unitTesting), _simpleBootTest(simpleBootTest) {
...
void QGCApplication::init() {
...
#ifdef QGC_VIEWER3D
    qmlRegisterType<LogFileHelper>("QGroundControl.LogHelper", 1, 0, "LogFileHelper");
    ...
    (void)qmlRegisterSingletonType<ShapeFileHelper>("QGroundControl.ShapeFileHelper", 1, 0, "ShapeFileHelper",
                                                    [](QQmlEngine *, QJSEngine *) { return new ShapeFileHelper(); });
